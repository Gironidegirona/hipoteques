...
with tab1:
    ...
    with col2:
        st.header("Banc")
        pcentrada = float(st.text_input("Entrada minima exigida [%]", value="20"))
        anys = float(st.text_input("Anys hipoteca", value="30"))
        rate = float(st.text_input("Interes [%]", value="2.75"))

    with col3:

        entrada, impostos, notaris = despeses(vivenda, pcentrada = pcentrada/100., pc_notari = pc_notari, pc_impostos = pc_impostos)  

        deute = vivenda+reforma-entrada
        cuotamensual = round(calculateCuota(deute, rate, anys = anys),1)
        estalvisnecessaris = round(entrada+impostos+notaris,1)
        costtotalvivenda = round(anys*12*cuotamensual/1000.+ estalvisnecessaris,1)
        ...
        frase = "Cost total vivenda: "
        spaces = fillSpaces(maxLen-len(frase))
        st.success(f"{frase}{spaces}{costtotalvivenda}k€")
    ...
    ncuotamensual = round(calculateCuota(deute, rate, anys = anys),1)

    st.header("Quotes en funció de interès")
    n = numSteps
    dn = deltaSteps
